...
그러나 협조 기반으로 작동하는 멀티태스킹도 있습니다. 어떤 작업이 CPU 시간을 필요로 할 때 이 작업으로 컨텍스트 스위칭을 하는 것입니다. 이는 *코드 상에서 명시적으로 yield(양도)를 하겠다고 명시*해야 합니다. 특정 프레임워크는 yield point에서 태스크를 전환하는 것을 지원하기도 힙니다. 16-bit 윈도우 프로그램이나 원조 Mac OS는 이 방법을 사용했습니다.
